...
# ==========================================
# 1. COMPILATION STEP (MUST BE DONE FIRST)
# ==========================================
...
# Test 1: n=0 (should have 0 solutions)
...
# Validate n=4 specific solutions ("1 3 0 2" and "2 0 3 1")
echo "${BLUE}Validating n=4 specific solutions...${RESET}"
sort output4.txt > sorted4.txt
echo -e "1 3 0 2\n2 0 3 1" | sort > expected4.txt
if ! diff -q sorted4.txt expected4.txt >/dev/null; then
    echo "$(tput setaf 1)$(tput bold)FAIL: n=4 solutions do not match expected output.$(tput sgr 0)"
    echo "Expected:"
    cat expected4.txt
    echo "Got:"
    cat sorted4.txt
...
echo "${BLUE}Testing arguments error handling...${RESET}"
./n_queens_test > temp.txt 2>&1
# Note: depending on implementation, main might return 0 or 1. 
# Usually simply exiting is fine, but it should NOT crash.
# This check ensures it doesn't output valid solutions for empty args.
if [ -s temp.txt ] && grep -qE "[0-9]" temp.txt; then 
     # If it printed numbers, that's bad.
     echo "$(tput setaf 1)$(tput bold)FAIL: Should not print solutions with no arguments$(tput sgr 0)"
     rm -f n_queens_test output*.txt temp.txt
     exit 1
fi

# Cleanup
rm -f n_queens_test output*.txt sorted4.txt expected4.txt temp.txt
